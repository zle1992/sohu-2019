...
class EmotionProcessor(DataProcessor):
  ...
  def get_train_examples(self, data_dir):
    ...
    self.train_df.columns=['newsId', 'texts', 'entity', 'emotion']

    k = int(len(self.train_df)*0.05)
    train_df = self.train_df[:-k].sample(frac =1)
  ...
  def get_test_examples(self, data_dir):
    ...
    self.test_df.columns=['newsId', 'texts', 'entity', 'emotion']
  ...
  def save_predict(self,res,output_predict_file):
      def agg_emotion_res(df1):
        from collections import Counter

        gg = df1.groupby(['newsId','entity'])
        df_agg = gg['emotion'].apply(lambda x:' '.join(x)).reset_index()
        df_agg['emotion'] = gg['emotion'].apply(list).reset_index()['emotion']
        print(list(df_agg))
        df_agg['emotion']=df_agg['emotion'].map(lambda x:[i[0] for i in Counter(x).most_common(1)][0])
        
        
        feat_agg = ['entity','emotion',]
        gg = df_agg.groupby(['newsId'])
        df_agg2 = gg[feat_agg[0]].apply(lambda x:' '.join(x)).reset_index()
        for f in feat_agg:
          df_agg2[f] = gg[f].apply(list).reset_index()[f]
          df_agg2[f] = df_agg2[f].map(lambda x:','.join(x))
            
        return df_agg2
      #logging.info('emo pred:')
     
      self.test_df['emotion'] = res
      df_pred = self.test_df[['newsId','entity','emotion']]
      ...
      df_test[['newsId','entity','emotion']].to_csv(output_predict_file, sep='\t',index=False,header=False)
...
class EntityProcessor(DataProcessor):
  ...
  def save_predict(self,res,output_predict_file):
    ...
    self.test_df['label'] = res
    df_pred = self.test_df[['newsId','textb','label']]
    gg = df_pred[df_pred.label=='1'][['newsId','textb']].groupby(['newsId'])
    ...
    print(df_agg2.head())
    df_test = df_agg2
    df_test.columns=['newsId','entity']
    df_test['emotion'] ='POS'
    df_test['emotion'] =df_test.apply(lambda x:['POS']*len(x['entity'].split(',')),axis=1)
    df_test['emotion'] =df_test['emotion'].map(lambda x:','.join(x))
